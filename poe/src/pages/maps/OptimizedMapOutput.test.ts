...
describe("generateMapModRegex price filter", () => {
  ...
  test("keeps the price clause separate from a combined yield clause", () => {
    const result = generateMapModRegex(
      mapSettings({
        anyYield: true,
        quantity: "80",
    ...
    const clauses = Array.from(result.matchAll(/"([^"]+)"/g), (match) => match[1]);
    expect(clauses).toHaveLength(2);
    expect(clauses.filter((clause) => clause.includes("Note:"))).toHaveLength(1);
    expect(clauses.filter((clause) => !clause.includes("Note:"))).toHaveLength(1);
    expect(resultMatches(result, "Note: ~b/o 25 chaos")).toBe(true);
  });
});
...
describe("generatePriceNoteRegex - invalid input", () => {
  ...
  test.each([
    ...
    ["min above supported cap", {min: "1000", max: ""}],
    ["max above supported cap", {min: "", max: "1000"}],
