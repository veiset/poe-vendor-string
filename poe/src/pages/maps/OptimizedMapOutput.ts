...
import {generateIntegerRangeRegex, generateNumberRegex} from "@shared/core/regex/GenerateNumberRegex";
...
export function generatePriceNoteRegex(options: PriceNoteOptions): string {
  ...
  if (maxRaw.length === 0) {
    if (priceAmount(minRaw) > PRICE_AMOUNT_MAX) return "";
    ...
    if (rawMin > PRICE_AMOUNT_MAX || rawMax > PRICE_AMOUNT_MAX || rawMin > rawMax) return "";
