...
export function generateMapModRegex(settings: MapSettings, regex: Regex<any>, language: RepoeLanguageKey): string {
  ...
  const scarabs = addQuantifier(statRegex.scarabs, generateNumberRegex(settings.scarabs, settings.optimizeQuant));
  ...
  const result = `${exclusions} ${inclusions} ${quantity} ${packsize} ${itemRarity} ${currency} ${scarabs} ${quality} ${rarity} ${mapDrop} ${corrupted} ${unidentified}`
