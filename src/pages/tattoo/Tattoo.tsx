...
import { tattooRegex } from "../../generated/GeneratedTattooRegex";
...
const Tattoo = () => {
    ...
    const [minChaosValue, setMinChaosValue] = useState<string>(profile.tattoo.minValue);
    const [maxChaosValue, setMaxChaosValue] = useState<string>(profile.tattoo.maxValue);
    ...
    useEffect(() => {
        ...
        data.then((d) => {
            const lineMap = new Map(d.lines.map((l) => [l.id, l.primaryValue]));
            const nameToIdMap = new Map(d.items?.map((i) => [i.name, i.id]) ?? []);

            // Map regexes to prices
            const pricedRegex: TattooPriceRegex[] = tattooRegex.map((t) => {
                const id = nameToIdMap.get(t.tattoo);
                const price = id ? (lineMap.get(id) ?? 0) : 0;
                return {
                    name: t.tattoo,
